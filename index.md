...
_Adventure Time_ is an award winning cartoon series created by Pendleton Ward that began airing in 2010 and wrapped its first series in 2018 (there are several spin-offs and mini-series). The show follows the adventures of Finn the Human and Jake the Dog, adoptive brothers who serve as the de-facto knights of the magical Land of Ooo. In the first season, Finn is 12 years old, black-and-white thinking, violence seeking, and, as far as we know, the only living human. By the end of season 10, the last season of the series, Finn is 17, wisened, resorts to communication over violence, and has survived depression, break-ups, an absent father, and a Lich. The first season is full of juvenile jokes and pre-pubescent themes (Finn blushes around the older-than-him princess and enjoys a good fart joke). While there is no lack of silly jokes and scenarios in the final seasons, the show begins to   incorporates themes of maturation, sexuality, and existentialism that a casual viewer of the first season would have never expected.
...
We will use the [BeautifulSoup library](https://beautiful-soup-4.readthedocs.io/en/latest/) to scrape the transcripts from the [_Adventure Time_ Wiki](navigate to https://adventuretime.fandom.com/wiki/List_of_episodes), use loops to clean, then save the data as individual sentences in a csv file.
...
for x in s1_trans_str:
    ...
    x = x.replace('(', '')
    x = x.replace(')', '')
    x = re.sub('<.+?>', '', x) # origional download puts "<>"  around speakers name
...
# first, on the toolbar, where it says RAM DISK, change the runtime type to GPU.

#%pip install transformers datasets trl torch # run this on it's own first, safe to leave commented out until you're absolutely ready
...
# load in the csv file containing lines from the first season transcript
...
# 2. those tokens are received as inputs by the model
# 3. the model processes the tokens, re-training itself
# 4. the model, when prompted, outputs tokens
# 5. those tokens become text
...
training_params = SFTConfig(
    ...
    weight_decay = 0.001, # regularizes the parameters to avoid overfitting (overfit = output is similar or the same as the training data)
...
# trainer.train() # safe to leave commented out until you're absolutely ready
```
...
```py
# run only if starting a new session
# %%capture
...
pipe = pipeline('text-generation', model=model, tokenizer=tokenizer, max_length=50) # max length refers to max tokens in output
...
```
## Repeat the process
Re-start the process starting from [fine-tuning](https://xxsteinxx.github.io/chara-analysis_at/#:~:text=Fine%2Dtuning%20the%20Model) for season 10, being sure to save the model (named appropriately) and the final csv file containing all outputs. Below is an embedding of a color-coded spreadsheet of the author's outputs for season 1 and season 10 to allow for easier comparison of outputs.
...
If you have any comments, questions, or concerns, please reach out to the author, Jace Steiner, at [esteiner@pratt.edu](mailto:esteiner@pratt.edu)